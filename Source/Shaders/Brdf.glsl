...
vec3 ImportanceSampleGGX_VNDF(vec2 u, float roughness, vec3 V, mat3 basis)
{
    ...
    float r = sqrt(u.x * _globalUBO.pt_ndf_trim);
...
{
    projected_lf *= _globalUBO.flt_scale_lf;
    high_freq *= _globalUBO.flt_scale_hf;
    specular *= _globalUBO.flt_scale_spec;
    ...
    if (_globalUBO.flt_fixed_albedo != 0)
        albedo = vec3(_globalUBO.flt_fixed_albedo);

    vec3 final_color = (projected_lf.rgb + high_freq.rgb) * albedo + specular.rgb;
    
    final_color *= throughput;

    transparent *= _globalUBO.flt_scale_overlay;
