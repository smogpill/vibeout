...
bool rectilinear_forward(vec3 view_pos, out vec2 screen_pos, out float distance, bool previous)
{
	vec4 clip_pos;
	if (previous)
		clip_pos = _globalUBO._prevProj * vec4(view_pos, 1);
	else
		clip_pos = _globalUBO._proj * vec4(view_pos, 1);
...
vec3 rectlinear_reverse(vec2 screen_pos, float distance, bool previous)
{
	...
	vec3 view_dir;
	if (previous)
		view_dir = normalize((_globalUBO._prevInvProj * clip_pos).xyz);
	else
		view_dir = normalize((_globalUBO._invProj * clip_pos).xyz);
...
{
	return previous ? _globalUBO._projection_fov_scale_prev : _globalUBO._projection_fov_scale;
}
...
{
	switch (_globalUBO.pt_projection)
	{
	default:
